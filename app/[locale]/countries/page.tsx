...
function getCountryDisplayName(name: string, code?: string, locale = 'fr') {
  ...
  if (/^[A-Z]{2}$/.test(codeCandidate)) {
    try {
      return new Intl.DisplayNames([locale], { type: 'region' }).of(codeCandidate) || trimmed
    } catch {
      return trimmed
    }
  }
      ...
      <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {filteredCountries.length > 0 ? (
          filteredCountries.map((c) => {
            const displayName = getCountryDisplayName(c.name, c.code, locale)
              ...
              <Link key={c.key} href={`/country/${slugify(c.name)}`} className="group flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 transition hover:border-slate-300 hover:shadow-md">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-slate-500">{c.count} {common('channels')}</div>
                  </div>
                </div>
              </Link>
            )
