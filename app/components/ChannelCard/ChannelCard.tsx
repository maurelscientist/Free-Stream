...
export default function ChannelCard({ id, name, country, logo, categories, streams }: Props) {
  ...
  return (
    <article onClick={openChannel} className="relative cursor-pointer border rounded-lg p-4 hover:shadow-lg transition-shadow">
      
      <div className="flex items-center space-x-4">
        <div className="w-16 h-16 bg-white border border-slate-200 rounded overflow-hidden flex items-center justify-center shadow-sm">
          {logo && !imageError ? (
            <img
              src={logo}
              alt={`${name} logo`}
            ...
            <span className="text-sm font-semibold uppercase tracking-wide text-slate-600">{name.slice(0, 2)}</span>
          )}
        </div>
        <div>
          ...
          <div className="text-sm text-slate-500">{getCountryName(country || FALLBACK_COUNTRY, locale)} • {categories?.[0] ? getCategoryName(categories[0], locale) : '—'}</div>
        </div>
      </div>
